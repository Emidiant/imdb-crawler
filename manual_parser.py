...
def main():
    ...
    data_films = []
    ia = Cinemagoer()
    for start in [0, 51, 101, 151, 201, 251]:
        if start != 0:
            response = requests.get(f"https://www.imdb.com/search/title/?genres={genre.lower()}&start={start}").content
        else:
            response = requests.get(f"https://www.imdb.com/search/title/?genres={genre.lower()}").content
